...
class MultiColor with ChangeNotifier {
  ...
  inisialData() async {
    Uri url = Uri.parse(
      'https://http-req-9401d-default-rtdb.firebaseio.com/colors.json',
    );
  ...
  bool checkAllStatus() {
  if (_colors.isEmpty) return false;
  return _colors.every((element) => element.status == true);
}
  ...
  void deleteColor(String id) async {
    Uri url = Uri.parse(
      'https://http-req-9401d-default-rtdb.firebaseio.com/colors/$id.json',
    );
